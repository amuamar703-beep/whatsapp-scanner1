...
async def start_exploration_callback(callback: types.CallbackQuery, state: FSMContext):
...
async def process_source(message: types.Message, state: FSMContext, user_id: int):
...
async def run_exploration(callback: types.CallbackQuery, state: FSMContext):
    await callback.answer()
    ...
    try:
        ...
        await callback.message.edit_text(
            f"❌ حدث خطأ أثناء بدء الاستكشاف: {str(e)}"
        )

async def change_source(callback: types.CallbackQuery, state: FSMContext):
    await callback.answer()
    await state.finish()
...
async def show_results(callback: types.CallbackQuery):
    ...
    try:
        async with get_db() as db:
            ...
            total_pages = max(1, (unique_links + 19) // 20)
            ...
            await callback.message.edit_text(
                ...
                f"الصفحة 1 / {total_pages}",
                reply_markup=results_pagination_keyboard(job_id, 1, total_pages)
            )
    except Exception as e:
        await callback.message.edit_text(
            f"❌ حدث خطأ: {str(e)}",
